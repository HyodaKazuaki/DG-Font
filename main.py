...
parser.add_argument(
    "--workers", default=4, type=int, help="the number of workers of data loader"
)
...
parser.add_argument(
    "--iters", default=1000, type=int, help="Total number of iterations per epoch"
)
parser.add_argument(
    "--batch_size", default=32, type=int, help="Batch size for training"
)
parser.add_argument(
    "--val_num", default=190, type=int, help="Number of test images for each style"
)
parser.add_argument(
    "--val_batch",
    default=10,
...
parser.add_argument(
    "--output_k", default=400, type=int, help="Total number of classes to use"
)  # It means the number of characters
...
parser.add_argument(
    "--w_rec", default=0.1, type=float, help="Coefficient of Rec. loss of G"
)
parser.add_argument(
    "--w_adv", default=1.0, type=float, help="Coefficient of Adv. loss of G"
)
parser.add_argument(
    "--w_vec",
    default=0.01,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Classes to use
    args.att_to_use = [i for i in range(args.num_cls)]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch == args.ema_start and "GAN" in args.train_mode:
            if args.distributed:
                networks["G_EMA"].module.load_state_dict(
                    networks["G"].module.state_dict()
                )
            else:
...
def build_model(args):
    ...
    if "C" in args.to_train:
        networks["C"] = GuidingNet(
            args.img_size, {"cont": args.sty_dim, "disc": args.output_k}
        )
    ...
    if args.distributed:
        if args.gpu is not None:
            ...
            for name, net in networks.items():
                ...
                networks[name] = torch.nn.parallel.DistributedDataParallel(
                    net_tmp, device_ids=[args.gpu], output_device=args.gpu
                )
        else:
            for name, net in networks.items():
                net_tmp = net.cuda()
    ...
    if "C" in args.to_train:
        opts["C"] = torch.optim.Adam(
            networks["C"].module.parameters()
            if args.distributed
            else networks["C"].parameters(),
            1e-4,
            weight_decay=0.001,
        )
        if args.distributed:
    ...
    if "D" in args.to_train:
        opts["D"] = torch.optim.RMSprop(
            networks["D"].module.parameters()
            if args.distributed
            else networks["D"].parameters(),
            1e-4,
            weight_decay=0.0001,
        )
    if "G" in args.to_train:
        opts["G"] = torch.optim.RMSprop(
            networks["G"].module.parameters()
            if args.distributed
            else networks["G"].parameters(),
            1e-4,
            weight_decay=0.0001,
        )
